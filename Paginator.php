...
class Paginator{
	...
	public function get($opt=array()){
		...
		if($this->options['alphabetic']){
			...
			if($this->tot<=$this->options['limit_left']+$this->options['limit_right']){
				...
				$paginazione_start = $this->options['pag']-$this->options['limit_left']; if($paginazione_start<1) $paginazione_start = 1;
				$paginazione_end = $this->options['pag']+$this->options['limit_right']; if($paginazione_end>$this->tot) $paginazione_end = $this->tot;
			}
		}
		...
		for($cp=$paginazione_start;$cp<=$paginazione_end;$cp++){
			if($this->options['alphabetic']){
				$cpt = chr($cp);
				if(!$this->options['englishLetters'] and !in_array(strtolower($cpt), array('x', 'y', 'w', 'j', 'k')))
					continue;
			}else{
				$cpt = $cp;
			}
			$current = $cpt==$this->options['pag'];
			$pagine[] = array('text' => $cpt, 'p' => $cpt, 'current' => $current, 'special' => false);
		}
	...
	public function render($opt=array(), $return=false){
		$pages = $this->get();
		if(count($pages)<=1)
			return '';
